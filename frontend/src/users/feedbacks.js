...
export default function Feedbacks({ user }) {
  ...
  // ✅ Logged in check (prop OR localStorage)
  ...
  const handleWriteFeedback = () => {
    if (!isLoggedIn) {
      alert("Please login first to write feedback.");
      navigate("/login");
      return;
    }
  ...
  const filteredFeedbacks = feedbacks.filter((fb) => {
    ...
    if (filters.withPhoto !== null) {
      if (filters.withPhoto && (!fb.photos || fb.photos.length === 0))
        return false;
      if (!filters.withPhoto && fb.photos && fb.photos.length > 0)
        return false;
    }
  ...
  const totalPages = Math.ceil(
    filteredFeedbacks.length / FEEDBACKS_PER_PAGE
  );

  if (loading)
    return <p className="status-message loading">Loading feedbacks...</p>;
  ...
  return (
    <div className="feedbacks-page">
      ...
      <div className="feedbacks-content-container">
        ...
        {/* ✅ WRITE BUTTON (ONLY WORKS WHEN LOGGED IN) */}
        <div className="write-feedback-btn">
          <button onClick={handleWriteFeedback}>
            ✍️ Write Feedback
          </button>
        </div>
