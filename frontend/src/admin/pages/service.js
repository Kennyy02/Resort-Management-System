...
function AdminServices() {
  ...
  const handleAdd = async () => {
    ...
    try {
      const res = await fetch(`${API_BASE_URL}/api/services`, { method: 'POST', body: data });
  ...
  const handleUpdate = async () => {
    ...
    try {
      const res = await fetch(`${API_BASE_URL}/api/services/${editingServiceId}`, { method: 'PUT', body: data });
  ...
  if (loading) return <div className="admin-services-container">Loading services...</div>;
  if (error) return <div className="admin-services-container">Error: {error}</div>;
    ...
    <div className="admin-services-container">
      ...
      <div className="service-form">
        ...
        <div className="form-row">
          ...
          <label>Name</label>
          <input name="name" type="text" placeholder="Name / Title" value={formData.name} onChange={handleChange} />
        </div>
        ...
        <div className="form-row">
          ...
          {formData.type !== "payment" && (
            <>
              <label>Price</label>
              <input name="price" type="number" placeholder="Price" value={formData.price} onChange={handleChange} min="0" step="0.01" />
            </>
          )}
        </div>
          ...
          services.map(s => (
            <li key={s.id} className="service-item">
              <div className="service-details-text">
                ...
                {s.type !== "payment" && <> - ₱{parseFloat(s.price).toFixed(2)}</>}
                <span className={`status-${s.status}`}> ({s.status})</span>
              </div>
              {s.image_url && <img src={`${API_BASE_URL}${s.image_url}`} alt={s.name} className="service-list-image" />}
