...
                  // FIX: Trim and filter photos array for clean file names
                  const photos = fb.photos ? fb.photos.split(",").map(p => p.trim()).filter(p => p) : [];
                  const displayedPhotos = photos.slice(0, 2); // Show max 2 photos
                  const remainingPhotosCount = photos.length - displayedPhotos.length;
                  
                  return (
                    <tr key={fb.id} className={`feedback-row ${fb.deleted ? "deleted-row" : ""}`}>
                      <td>{fb.name}</td>
